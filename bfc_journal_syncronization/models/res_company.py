    ...
    def sync_with_master_company_journals(self):
        ...
        for company in target_companies:
            ...
            for master in master_journals:
                ...
                if existing_journal:
                    existing_journal.with_context(account_journal_skip_alias_sync=True).write(vals)
                    continue
            ...
            if to_create:
                ...
                for journal, (_, child_default_account_id, master_default_account) in zip(created_journals, to_create):
                    journal._sync_translated_fields(journal.parent_journal_id)
                    if not child_default_account_id and master_default_account:
                        journal.message_post(
                            body=self.env._(
                                "Journal created but Default Income Account were not synchronized because Account is missing for this company."
                            )
                        )
