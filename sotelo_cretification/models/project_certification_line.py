...
class ProjectCertificationLine(models.Model):
    ...
    def _compute_prev_work_status_percentage(self):
        for record in self:
            ...
            if not cert or not cert.project_id or not record.sale_line_id:
                continue
            ...
            prev_cert = previous_certs[0]
            prev_line = prev_cert.certification_line_ids.filtered(
    ...
    def _compute_certification_percentage(self):
        for record in self:
            if record.product_uom_qty:
                record.certification_percentage = (record.certification_uds / record.product_uom_qty)
            else:
                record.certification_percentage = 0
