...
class ProjectCertification(models.Model):
    ...
    is_first_certification = fields.Boolean(
        compute="_compute_id_first_certfication",
    ...
    def _compute_partner_related_fields(self):
        for certificate in self:
            ...
            if partner:
                ...
                certificate.payment_term_id = partner.with_company(
    ...
    @api.depends('project_id', 'project_id.certification_ids')
    def _compute_id_first_certfication(self):
        for record in self:
            if not record.project_id:
                record.is_first_certification = False
            first_certificate = record.project_id.certification_ids.sorted("create_date")[:1]
            record.is_first_certification = record.id == first_certificate.id
    ...
    def _compute_certification_totals(self):
        for record in self.filtered(lambda rec: rec.state != 'confirmed'):
    ...
    def action_update_certification_line(self):
        for record in self:
            ...
            for line in sale_orders.mapped("order_line").filtered(lambda line: line.display_type in [False, 'line_section']):
    ...
    def _prepare_invoice_vals(self, previous_certs):
        return {
            ...
            "invoice_line_ids": self._prepare_invoice_lines(previous_certs),
    ...
    def action_create_invoice(self):
        self.ensure_one()
        if not self.project_id.certification_product or not self.project_id.invoice_deduction_product:
            raise UserError(
                self.env._("Please configure Certification Product and Invoice Deduction Product on the project.")
            )
        previous_certs = self._get_previous_confirmed_certifications()
        previous_amount = sum(previous_certs.mapped("total_to_invoice"))
    ...
    def write(self, vals):
        ...
        old_status = self.billable_status
        res = super().write(vals)
        self._post_billable_status_change(old_status, vals['billable_status'])
        return res
